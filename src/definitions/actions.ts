...
const getSingleOrder = {
  ...
  description: 'Gets a single order based on the order ID',
  input: {
    schema: z.object({
      order_id: z.string().describe('The exact order ID.'),
    }),
    ui: {
      order_id: {
        title: 'Order ID',
        examples: ['1001', '1002'],  // Example order IDs for guidance
      },
    },
  },
  output: {
    schema: z.object({
      order: z.object({}).passthrough(),  // You might want to define this more strictly based on expected fields
    }),
  },
} satisfies ActionDefinition;


export const actions = { getProducts, getProductVariants, getCustomers, getCustomerOrders, getSingleOrder }
